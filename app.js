document.addEventListener("DOMContentLoaded", () => {
  ...
  const escudosSpainGrid = document.getElementById("escudos-spain");
  const escudosAmericasGrid = document.getElementById("escudos-americas");
  const presidentesSpainGrid = document.getElementById("presidentes-spain");
  const presidentesAmericasGrid = document.getElementById("presidentes-americas");
  const superGrid = document.createElement("div"); superGrid.id="super-grid"; superGrid.classList.add("grid");
  ...
  function mostrarAlbum() {
    ...
    if(!duplicadasBtn){
      ...
      duplicadasBtn.onclick = () => {
        ...
        if(ganancia>0){ monedas+=ganancia; updateMonedas(); localStorage.setItem("album_queens", JSON.stringify(album)); mostrarAlbum(); alert(`Has vendido duplicadas y recibido ${ganancia} monedas.`);}
    ...
    CARDS.forEach(card=>{
      ...
      if(card.tipo==="escudo"){
        if(card.id<=12) escudosSpainGrid.appendChild(div);
        else escudosAmericasGrid.appendChild(div);
      } else if(card.tipo==="presidenta"){
        if(card.id<=24) presidentesSpainGrid.appendChild(div);
        else presidentesAmericasGrid.appendChild(div);
  ...
  function abrirSobre(){
    ...
    for(let i=0;i<5;i++){
      ...
      const img=document.createElement("img");
      img.src=c.imagen; img.alt=c.nombre;
  ...
  // Códigos
  const CODIGOS={"AroneyGonzalez":10000,"paulaanavas_gg":100000,"pinkgoldqueen":10000,"annatan99_":10000,"AndreaChini":10000,"NarcisBoza":10000,"izandhh":10000,"sronzero":10000,"ZonaRayo":10000,"Porcinismoo":10000,"NikolRamos":10000,"ZonaMostoles":10000,"CZXR":10000};
